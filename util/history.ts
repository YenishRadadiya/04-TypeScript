const history_button = document.getElementById('btn-history') as HTMLButtonElement | null;
const history_div = document.getElementById('display_history') as HTMLDivElement | null;
const history_content = document.getElementById('history_content') as HTMLDivElement | null;
...
function load_history() {
    ...
    if (hist) {
        let historyArray: Expression[] = JSON.parse(hist); // Convert back to array
        historyArray.forEach((exp: Expression) => { // Explicitly define exp type
